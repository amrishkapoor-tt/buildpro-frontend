...
const Dashboard = ({ projectId, project, token, onNavigate }) => {
  ...
  const loadWorkflowStats = async () => {
    try {
      const response = await fetch(`${API_URL}/workflows/stats/project/${projectId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const loadWorkflowTasks = async () => {
    try {
      const response = await fetch(`${API_URL}/workflows/tasks/my-tasks?project_id=${projectId}&limit=5`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
